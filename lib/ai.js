...
/** One Groq call → chapters for a single transcript window. */
async function runChapters(lines, duration, note = "") {
  const res = await groqFetch(`${GROQ_BASE}/chat/completions`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${apiKey()}`,
...
async function detectChapters(transcript, duration, opts = {}) {
  ...
  let raw = [];
  if (chunks.length <= 1) {
    raw = await runChapters(chunks[0]?.lines || [], duration);
  } else {
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        raw.push(...(await runChapters(ch.lines, duration, note)));
      } catch (e) {
...
/** Map step: compress one transcript chunk to bullet points. */
async function digestChunk(lines) {
  const res = await groqFetch(`${GROQ_BASE}/chat/completions`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${apiKey()}`,
...
async function repurpose(transcript, meta = {}, onProgress, opts = {}) {
  const llm = opts.llm; // injected big-context provider (Gemini), optional
  const chunks = chunkSegmentLines(transcript.segments || []);
  let digest;
  if (llm) {
    // Big-context path: feed the whole transcript straight to the writer in ONE
    // call. The Groq map-reduce below fires one request PER chunk — dozens on a
    // multi-hour video — which on a rate-limited free tier takes minutes and
    // makes the browser give up ("Failed to fetch"). Gemini's huge context does
    // it in a single fast, cheap call.
    ...
    // Map: summarize each chunk to bullets, then reduce over the bullets.
    const bullets = [];
    for (let i = 0; i < chunks.length; i++) {
      if (onProgress) onProgress(i, chunks.length + 1);
      try {
        const b = await digestChunk(chunks[i].lines);
