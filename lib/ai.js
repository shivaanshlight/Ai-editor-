 ...
 */
async function groqFetch(url, init, onRetry) {
  let res;
...
async function writePack(digest, meta) {
  const userMsg = [
 ...
 */
async function repurpose(transcript, meta = {}, onProgress) {
  const chunks = chunkSegmentLines(transcript.segments || []);
  let digest;
  if (chunks.length <= 1) {
  ...
  const raw = await writePack(digest, meta);
