...
function apiKey() {
  ...
  if (!key) throw new Error("GROQ_API_KEY missing — create a .env file (see .env.example).");
  return key;
}
...
async function transcribe(audioPath) {
  ...
  if (!data.words?.length && !data.segments?.length)
    throw new Error("Whisper returned no speech — is there talking in this video?");
  return data;
}
...
async function planEdit(transcript, instruction, targetDuration, duration) {
  ...
  if (!res.ok) {
    const body = await res.text();
    throw new Error(`Edit planning failed (${res.status}): ${body.slice(0, 300)}`);
  }
  const data = await res.json();
  let plan;
  try {
  ...
  if (!Array.isArray(plan.segments) || !plan.segments.length)
    throw new Error("The LLM returned no segments to keep — try a different instruction.");
  return plan;
}
...
function validateEdl(segments, duration, words = [], opts = {}) {
  ...
  let clean = segments
    ...
    .filter((s) => isFinite(s.start) && isFinite(s.end) && s.end - s.start >= minKeep)
    .sort((a, b) => a.start - b.start);

  const merged = [];
  for (const s of clean) {
    const last = merged[merged.length - 1];
    if (last && s.start - last.end < minCut) last.end = Math.max(last.end, s.end);
    else merged.push({ ...s });
  }
  if (!merged.length) throw new Error("Edit plan had no usable segments after validation.");
  return merged;
}

const FILLER_WORDS = new Set(["um", "uh", "umm", "uhh", "er", "erm", "hmm", "mmm", "ah", "uhm"]);
...
function removeFillers(segments, words) {
  ...
  for (const seg of segments) {
    ...
    for (const f of fillers) {
      const cutStart = f.start - 0.04, cutEnd = f.end + 0.04;
      pieces = pieces.flatMap((p) => {
        if (cutEnd <= p.start || cutStart >= p.end) return [p]; // no overlap
        const result = [];
        if (cutStart - p.start >= 0.3) result.push({ start: p.start, end: cutStart });
...
function shrinkPauses(segments, words, { maxGap = 0.8, keep = 0.4 } = {}) {
  const out = [];
  for (const seg of segments) {
    const inside = words.filter((w) => w.start >= seg.start - 0.05 && w.end <= seg.end + 0.05);
    if (inside.length < 2) { out.push(seg); continue; }
    let pieces = [];
...
module.exports = { transcribe, planEdit, validateEdl, removeFillers, shrinkPauses };
