...
const DEFAULTS = {
  ...
  dramaticSalience: 75,
  lowEnergyZ: -1.0, // energyZ at/below this = a low-energy unit
  deadEnergyMinRun: 2, // this many low-energy units in a row = "dead energy"
};
...
/**
 * Dead energy = a RUN of consecutive low-energy units (energyZ ≤ lowEnergyZ).
 * A single quiet word isn't dead air; a sustained flatline is. This is a SOFT
 * signal (flag only) — a hushed, meaningful beat is also low-energy, so the LLM
 * score still gets the final say. Downstream applies only a mild cut-nudge.
 */
function flagDeadEnergy(units, opts = {}) {
  const o = { ...DEFAULTS, ...opts };
  let run = [];
  const flush = () => {
    if (run.length >= o.deadEnergyMinRun) {
      for (const u of run) {
        u.deadEnergy = true;
        if (!u.flags.includes("dead energy")) u.flags.push("dead energy");
      }
    }
    run = [];
  };
  for (const u of units) {
    if ((u.energyZ ?? 0) <= o.lowEnergyZ) run.push(u);
    else flush();
  }
  flush();
  return units;
