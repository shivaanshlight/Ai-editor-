...
async function enginePlan(p) {
  ...
  const scored = await scoreUnits(units, {
    embed: p.embed, // embeddings+signals is the default tier when provided
    llm: p.llm,
    ...
    onProgress: (done, total) =>
      on(`scoring every moment — ${done} of ${total}`, 0.1 + 0.7 * (done / Math.max(1, total))),
  });
  ...
  for (const u of units) {
    ...
    // Repetition: the verbose restatement of a nearby point — nudge toward
    // cutting it so the crisper version survives (mild, gated on score < 75).
    if (u.flags.includes("repetition") && (u.score ?? 65) < 75) {
      u.score = Math.max(0, Math.round((u.score ?? 65) - 8));
      u.reason = (u.reason ? u.reason + " · " : "") + "repeats a nearby point";
    }
    // Emphasis / laugh = a genuine moment. Boost so real highlights survive the
    // cut and seed the shorts. Laughter is the strongest signal; raw high energy
    // is a gentler nudge. Never pushes an already-top unit over 100.
    if (u.flags.includes("laugh")) {
      u.score = Math.min(100, Math.round((u.score ?? 65) + 12));
      u.moment = true;
    } else if (u.highEnergy && (u.score ?? 0) < 85) {
      u.score = Math.min(100, Math.round((u.score ?? 65) + 5));
      u.moment = true;
    }
  }
  ...
  const selOpts = p.targetDuration
    ? { mode: "condense", targetDuration: p.targetDuration, lambda: 6, duration: p.duration, junkFloor: JUNK_FLOOR, noRestore: !!p.noRestore }
    : { mode: "tighten", keepCount, lambda: 6, duration: p.duration, junkFloor: JUNK_FLOOR, noRestore: !!p.noRestore };
