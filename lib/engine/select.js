...
function applyConstraints(units, o) {
  ...
  if (o.coverage) {
    ...
    for (const u of best.values()) {
      // Don't force-keep a whole chapter's "best" line when that best is itself
      // junk — an all-filler topic shouldn't get a guaranteed slot (that was
      // keeping scored-10/20 fillers alive despite the junk floor). Cover only
      // chapters that actually have something worth keeping.
      if (o.junkFloor && typeof u.score === "number" && u.score < o.junkFloor) continue;
      u.mustKeep = true;
    }
  }
