...
async function geminiChatJSON(messages, { temperature = 0.2 } = {}) {
  ...
  const body = {
    ...
    generationConfig: {
      ...
      // Scoring a big batch emits one JSON entry per unit; the model's default
      // output cap truncates that mid-array ("...\"reason\":\"Incomplete"), which
      // then parses as unbalanced JSON. Lift the cap so a full batch fits.
      maxOutputTokens: parseInt(process.env.GEMINI_MAX_OUTPUT_TOKENS || "32768"),
    },
  };
