...
async function geminiChatJSON(messages, { temperature = 0.2 } = {}) {
  ...
  const body = {
    ...
    generationConfig: {
      ...
      // Scoring a batch emits one JSON entry per unit; cap the reply generously
      // so it doesn't truncate mid-array, but not so high that a runaway reply
      // costs a fortune. 8192 fits ~150 score entries with room to spare.
      maxOutputTokens: parseInt(process.env.GEMINI_MAX_OUTPUT_TOKENS || "8192"),
      // CRITICAL COST + RELIABILITY FIX: gemini-3.x defaults to "medium"
      // thinking. Thinking tokens are billed at the $9/M OUTPUT rate AND are
      // consumed from the output budget — so they both (a) run up surprise
      // costs (₹100+ on a few calls) and (b) starve the real answer, truncating
      // the JSON. Rating a line 0-100 needs zero chain-of-thought, so set the
      // thinking budget to 0. This is the single biggest cost saver here.
      thinkingConfig: {
