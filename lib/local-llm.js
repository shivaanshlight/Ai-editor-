...
async function chatJSON(messages, { temperature = 0.2 } = {}) {
  const res = await fetch(`${BASE}/api/chat`, {
    ...
    body: JSON.stringify({
      ...
      stream: false,
      // Ollama defaults num_ctx to 2048, which silently truncates longer
      // prompts (batch scoring, content-kit digests). qwen2.5:3b handles 32k;
      // 8192 covers our prompts without eating much VRAM.
      options: { temperature, num_ctx: parseInt(process.env.LOCAL_LLM_CTX || "8192") },
    }),
