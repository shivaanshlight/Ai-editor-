...
const TIMEOUT_MS = parseInt(process.env.LOCAL_LLM_TIMEOUT_MS || "180000");
...
async function chatJSON(messages, { temperature = 0.2 } = {}) {
  const res = await fetch(`${BASE}/api/chat`, {
    ...
    body: JSON.stringify({
      ...
      stream: false,
      // Ollama defaults num_ctx to 2048, which truncates longer prompts, but a
      // big context balloons the KV cache and on a 4GB card (RTX 3050) spills
      // the model to CPU — which crawls and hits the timeout. 4096 fits our
      // batched prompts while staying in VRAM. Bump LOCAL_LLM_CTX only if you
      // have more VRAM to spare.
      options: { temperature, num_ctx: parseInt(process.env.LOCAL_LLM_CTX || "4096") },
    }),
