/**
 * lib/media.js — audio extraction, captions, music with auto-ducking.
 */
...
/**
 * Compute a downsampled audio waveform (array of 0..1 peaks) for the timeline
 * UI — the same thing Premiere/DaVinci draw under each clip. Decodes to mono
 * low-rate PCM on stdout and takes the max amplitude per bucket. Runs entirely
 * locally (ffmpeg); no upload.
 */
function computeWaveform(videoPath, buckets = 2000) {
  return new Promise((resolve, reject) => {
    const p = spawn("ffmpeg", [
      "-hide_banner", "-v", "error",
      "-i", videoPath,
      "-vn", "-ac", "1", "-ar", "4000", // mono, 4kHz — plenty for a visual
      "-f", "s16le", "-",
    ]);
    const chunks = [];
    let stderr = "";
    ...
    p.on("error", reject);
    p.on("close", (code) => {
      if (code !== 0) return reject(new Error("ffmpeg waveform failed: " + stderr.slice(-400)));
      const buf = Buffer.concat(chunks);
      const total = Math.floor(buf.length / 2); // int16 samples
      if (!total) return resolve([]);
      const per = Math.max(1, Math.floor(total / buckets));
      const peaks = [];
      for (let i = 0; i < total; i += per) {
        let max = 0;
        const end = Math.min(total, i + per);
        for (let j = i; j < end; j++) {
...
/** Extract full-length mono audio (no size cap) for diarization/other APIs. */
async function extractAudioFull(videoPath, audioPath) {
  await ff([
    "-i",
    videoPath,
    "-vn",
...
/**
 * Map a whole word onto the output timeline, tolerantly: if a word straddles a
 * cut boundary but most of it (>=40%) survives inside one kept segment, clamp
 * it into that segment instead of dropping it. Fixes captions vanishing around
 * every cut point.
 */
function remapWord(w, keptSegments) {
  let offset = 0;
  const dur = Math.max(w.end - w.start, 0.01);
  for (const s of keptSegments) {
    const overlap = Math.min(w.end, s.end) - Math.max(w.start, s.start);
    if (overlap / dur >= 0.4) {
      const cs = Math.max(w.start, s.start),
        ce = Math.min(w.end, s.end);
      return { start: offset + (cs - s.start), end: offset + (ce - s.start) };
    }
    offset += s.end - s.start;
  }
  return null;
...
function buildSrt(words, keptSegments) {
  ...
  for (const w of words) {
    const m = remapWord(w, keptSegments);
    if (m && m.end > m.start)
      mapped.push({ word: w.word.trim(), start: m.start, end: m.end });
  }
 ...
 */
function buildAss(words, keptSegments, { vertical = false, width, height } = {}) {
  const mapped = [];
  for (const w of words) {
    const m = remapWord(w, keptSegments);
    if (m && m.end > m.start)
  ...
  // Match the caption canvas to the ACTUAL output size so libass places text
  // correctly regardless of resolution/orientation, and sit it near the bottom.
  const resX = vertical ? 720 : width || 1280;
  const resY = vertical ? 1280 : height || 720;
  const fontSize = Math.round(resY * (vertical ? 0.05 : 0.048));
  const outline = Math.max(2, Math.round(fontSize * 0.09));
  const marginV = Math.round(resY * 0.07); // ~7% up from the bottom edge
...
Style: Karaoke,Arial,${fontSize},&H00FFFFFF,&H00FFFFFF,&H00000000,&HA0000000,1,0,0,0,100,100,1,0,1,${outline},1,2,40,40,${marginV},1
...
/**
 * Speaker lower-thirds: a name-tag pinned bottom-left that changes to whoever is
 * talking. `renderSegs` is the OUTPUT edit (contiguous concat), `speakers` are
 * source-time utterances {speaker,start,end}, `names` maps a speaker label to a
 * display name. We find the dominant speaker inside each rendered segment, map
 * it to the output timeline, and merge consecutive same-name runs.
 */
function buildLowerThirds(renderSegs, speakers, names, { width, height, vertical = false } = {}) {
  if (!renderSegs?.length || !speakers?.length || !names) return null;

  const dominant = (a, b) => {
    const tally = {};
    for (const u of speakers) {
      const ov = Math.min(b, u.end) - Math.max(a, u.start);
      if (ov > 0) tally[u.speaker] = (tally[u.speaker] || 0) + ov;
    }
    let best = null,
      bestv = 0;
  ...
  // Walk the output timeline, one entry per rendered segment, merging runs.
  const runs = [];
  let acc = 0;
  for (const seg of renderSegs) {
    const len = seg.end - seg.start;
    const spk = dominant(seg.start, seg.end);
    const name = spk != null ? names[spk] : null;
    const last = runs[runs.length - 1];
    if (name && last && last.name === name && Math.abs(last.end - acc) < 1e-3)
      last.end = acc + len;
    else if (name) runs.push({ name, start: acc, end: acc + len });
    acc += len;
  }
  ...
  const resX = vertical ? 720 : width || 1280;
  const resY = vertical ? 1280 : height || 720;
  const fontSize = Math.round(resY * 0.038);
  const outline = Math.max(2, Math.round(fontSize * 0.12));
  const marginL = Math.round(resX * 0.045);
  const marginV = Math.round(resY * (vertical ? 0.14 : 0.1));
  ...
  // A cyan accent bar (▌) then the name, so it reads as a tactile lower-third.
  const events = runs
    .filter((r) => r.end - r.start > 0.4)
    .map(
      (r) =>
        `Dialogue: 0,${t(r.start)},${t(r.end)},Name,,0,0,0,,{\\c&HFFE037&}▌{\\c&HFFFFFF&} ${esc(r.name)}`,
    );
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: Name,Arial,${fontSize},&H00FFFFFF,&H00FFFFFF,&HC8000000,&HB4000000,1,0,0,0,100,100,0,0,3,${outline},1,1,${marginL},40,${marginV},1
...
/**
 * Legacy two-pass finishing step. Kept for reference; the live pipeline now
 * does captions/vertical/music inside cutVideo in a single encode.
 */
