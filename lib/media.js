...
async function extractAudio(videoPath, audioPath) {
  await ff(["-i", videoPath, "-vn", "-ar", "16000", "-ac", "1", "-b:a", "32k", audioPath]);
  const mb = fs.statSync(audioPath).size / 1024 / 1024;
  if (mb > 24)
    throw new Error(
      `Audio is ${mb.toFixed(0)} MB — over Groq's 25 MB limit. Video too long for one pass (chunking is a future upgrade).`
    );
}
...
function buildSrt(words, keptSegments) {
  ...
  for (const w of mapped) {
    ...
    if (sentenceEnd && cur.words.length >= 2) { cues.push(cur); cur = null; }
  }
  ...
  for (let i = 0; i < cues.length; i++) {
    ...
    if (prev && cues[i].start < prev.end + 0.05) cues[i].start = prev.end + 0.05;
...
function buildAss(words, keptSegments, { vertical = false } = {}) {
  ...
  for (const w of mapped) {
    if (!cur) cur = { words: [w] };
    else if (cur.words.length >= 3 || w.start - cur.words[cur.words.length - 1].end > 0.6) {
      lines.push(cur);
      cur = { words: [w] };
    } else cur.words.push(w);
    if (/[.!?]$/.test(w.word) && cur.words.length >= 1) { lines.push(cur); cur = null; }
  }
  ...
  const t = (sec) => {
    ...
    const s = Math.floor(sec % 60), cs = Math.round((sec % 1) * 100);
  ...
  for (const line of lines) {
    ...
    for (let i = 0; i < ws.length; i++) {
      ...
      const text = ws
        .map((w, j) =>
          j === i ? `{\\c&H3BB4FF&\\fscx108\\fscy108}${w.word}{\\c&HFFFFFF&\\fscx100\\fscy100}` : w.word
        )
        .join(" ");
...
const CAPTION_STYLES = {
  ...
  bold:
    "FontName=Arial,FontSize=30,Bold=1,PrimaryColour=&H00FFFFFF,OutlineColour=&HCC000000,BorderStyle=1,Outline=3,Shadow=1,Alignment=2,MarginV=80",
};
 ...
 */
async function finishPass(inputPath, outputPath, { srtFile, assFile, captionStyle, musicPath, musicVol = 0.25, vertical = false }) {
  ...
  args.push(
    "-c:v", reencodeVideo ? "libx264" : "copy",
...
module.exports = { extractAudio, remapTime, buildSrt, buildAss, finishPass, CAPTION_STYLES };
