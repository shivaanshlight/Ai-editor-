...
async function cutVideo(input, output, segments, onLog, opts = {}) {
  ...
  const perInput = input.length + 40; // path + "-ss N -t N -i " flags/numbers
  const CMDLINE_BUDGET = 28000; // headroom under the 32767 Windows ceiling
  const safeMax = Math.max(60, Math.floor(CMDLINE_BUDGET / perInput));
  const MAX_SEEK_INPUTS = Math.min(
    parseInt(process.env.FAST_SEEK_MAX || "600"),
    safeMax,
  );
  const usingFastSeek =
