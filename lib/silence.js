...
async function cutVideo(input, output, segments, onLog, opts = {}) {
  ...
  segments.forEach((s, i) => {
    ...
    parts.push(
      `[0:v]trim=start=${s.start.toFixed(4)}:end=${s.end.toFixed(4)},setpts=PTS-STARTPTS${vf}[v${i}];`,
  ...
  const buildArgs = (enc) => {
    ...
    // Soft captions ride along as an extra input, muxed (not burned) below.
    let subIdx = null;
    if (softSubFile) {
      subIdx = musicPath ? 2 : 1;
      a.push("-i", softSubFile);
    }
