...
async function cutVideo(input, output, segments, onLog, opts = {}) {
  ...
  //
  // Cap raised from 240 → 600: a real long-form edit routinely lands 300–450
  // kept segments, and forcing those into full-decode (decode the ENTIRE
  // multi-hour source) was the single biggest render-time regression. 600
  // demuxer contexts is well within ffmpeg's practical range for a re-encode,
  // and the decode savings on a heavily-trimmed video dwarf the per-input seek
  // overhead. Override with FAST_SEEK_MAX or disable via DISABLE_FAST_SEEK.
  const MAX_SEEK_INPUTS = parseInt(process.env.FAST_SEEK_MAX || "600");
  const usingFastSeek =
  ...
  segments.forEach((s, i) => {
    ...
    // Fast seek: the input is already trimmed to this segment (via -ss/-t), so
    // no trim filter — just reset PTS. Legacy: one input, trim per segment.
  ...
  const buildArgs = (enc) => {
    ...
    if (usingFastSeek) {
      // One input per kept segment, seeked so only that region is decoded.
      for (const s of segments)
        a.push(
          "-ss",
          s.start.toFixed(4),
          "-t",
          (s.end - s.start).toFixed(4),
          "-i",
          input,
        );
    } else {
