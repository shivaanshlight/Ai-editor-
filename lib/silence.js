...
const { FFMPEG, FFPROBE } = require("./ffbin");
...
async function probe(file) {
  ...
  try {
    // Use -show_format -show_streams (universally supported) rather than a
    // narrow -show_entries selector. The `stream_side_data=rotation` selector
    // only exists in newer ffprobe builds; the bundled ffprobe-static ships an
    // older ffmpeg and rejects it ("No match for section 'stream_side_data'").
    // -show_streams returns side_data_list (with rotation) in JSON anyway, so
    // the rotation parsing below still works — on every ffprobe version.
    ({ code, stdout, stderr } = await run(FFPROBE, [
      "-v",
      "error",
      "-show_format",
      "-show_streams",
    ...
    // spawn itself failed (binary not found / not executable).
    throw new Error(
      `Could not run ffprobe (${FFPROBE}): ${e.message}. ` +
        `The bundled media tools didn't load — reinstall the app.`,
    );
  }
  if (code !== 0) {
    const detail = (stderr || "").trim().split("\n").slice(-3).join(" ");
    throw new Error(
      "ffprobe couldn't read this file" + (detail ? ` — ${detail}` : "."),
    );
  }
...
async function cutVideo(input, output, segments, onLog, opts = {}) {
  ...
  const {
    ...
    enhanceAudio = false,
    cwd = null,
    gains = null, // [{start,end,db}] per-clip volume from the timeline
  } = opts;
  // Per-segment gain (dB) from the timeline's draggable waveform. Look up by the
  // segment's midpoint against the source-time gain regions.
  const gainDbFor = (s) => {
    if (!Array.isArray(gains) || !gains.length) return 0;
    const mid = (s.start + s.end) / 2;
    for (const g of gains) if (mid >= g.start && mid <= g.end) return g.db || 0;
    return 0;
  };
  ...
  segments.forEach((s, i) => {
    ...
    const gdb = gainDbFor(s);
    const gain = gdb ? `,volume=${gdb.toFixed(1)}dB` : "";
    parts.push(
      `${vSrc}${vTrim}setpts=PTS-STARTPTS${vf}[v${i}];`,
      `${aSrc}${aTrim}asetpts=PTS-STARTPTS${gain}${fade}[a${i}];`,
    );
    labels.push(`[v${i}][a${i}]`);
  });
  parts.push(
