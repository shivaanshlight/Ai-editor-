...
// Supabase's realtime client needs a global WebSocket at construction time.
// Electron bundles Node 20, which has no native WebSocket, so createClient would
// throw and crash the whole app at startup. Provide one from `ws` if missing —
// the app never uses realtime; this just satisfies the constructor.
if (typeof globalThis.WebSocket === "undefined") {
  try {
    globalThis.WebSocket = require("ws");
  } catch {
    /* ws not installed — the try/catch below keeps the app alive anyway */
  }
}
...
if (!URL || !SERVICE_KEY || !DEV_USER_ID) {
  console.error(
    "Supabase not configured — running local-only (no cloud persistence).",
  );
}

// Supabase is OPTIONAL. Never let its init crash the app — on any failure the
// app runs local-only (in-memory jobs, local files).
let supabase = null;
if (URL && SERVICE_KEY) {
  try {
    supabase = createClient(URL, SERVICE_KEY, { auth: { persistSession: false } });
  } catch (e) {
    console.error("Supabase disabled (init failed):", e.message);
    supabase = null;
  }
}
...
const HEAVY = new Set(["_timer", "words", "transcriptText", "waveform"]);
...
/** Replace a job's passages with freshly embedded ones. */
async function savePassages(jobId, rows) {
  if (!supabase) return;
  await supabase.from("passages").delete().eq("job_id", jobId);
  const payload = rows.map((r) => ({
    job_id: jobId,
    start_s: r.start,
    end_s: r.end,
    text: r.text,
    embedding: "[" + r.embedding.join(",") + "]", // pgvector text literal
  }));
  for (let i = 0; i < payload.length; i += 200) {
    const { error } = await supabase
      .from("passages")
      .insert(payload.slice(i, i + 200));
    if (error) throw new Error("savePassages: " + error.message);
  }
}

/** Cosine search a job's passages for a query embedding. */
async function searchPassages(jobId, queryVec, k = 12) {
  if (!supabase) return [];
  ...
  if (error) throw new Error("searchPassages: " + error.message);
  return data || [];
}
