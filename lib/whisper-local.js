...
function binaryPath() {
  return firstExisting([
    ...
    // whisper.cpp renamed its CLI and left the OLD names as deprecation shims
    // that print a warning and exit WITHOUT transcribing. Prefer the current
    // real binary; the older names are kept only as last-resort fallbacks.
    path.join(ROOT, "bin", "whisper-whisper-cli.exe"),
...
async function transcribe(mediaPath, { onProgress, threads } = {}) {
  ...
  // Run the binary. If it turns out to be a deprecation shim ("The binary 'X'
  // is deprecated. Please use 'Y' instead."), read the recommended name from
  // its own output and retry once with that sibling binary — so an upstream
  // rename self-heals instead of silently falling back to rate-limited Groq.
  const runWhisper = (exe) =>
  ...
  try {
    ...
    const redirect = String(e.message).match(/use '([^']+)'\s+instead/i);
    const suggested = redirect && path.join(path.dirname(bin), redirect[1]);
    if (suggested && fs.existsSync(suggested)) {
      await runWhisper(suggested); // deprecation shim → real binary, retry once
    } else {
      throw new Error("whisper.cpp failed: " + String(e.message).slice(-400));
    }
  }
