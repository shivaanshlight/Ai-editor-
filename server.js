...
const { probe, detectSilence, buildKeepSegments, quantizeSegments, cutVideo } = require("./lib/silence");
const { transcribe, planEdit, validateEdl, removeFillers, shrinkPauses } = require("./lib/ai");
...
for (const f of fs.readdirSync(UPLOAD_DIR)) fs.unlink(path.join(UPLOAD_DIR, f), () => {});
...
app.post("/api/upload", upload.fields([{ name: "video", maxCount: 1 }, { name: "music", maxCount: 1 }]), (req, res) => {
  const video = req.files?.video?.[0];
  if (!video) return res.status(400).json({ error: "No video file received." });
  ...
  const job = {
    ...
    settings: {
      ...
      targetDuration: b.targetDuration ? clamp(parseFloat(b.targetDuration), 5, 36000) : null,
...
async function processJob(job) {
  ...
  if (job.mode === "silence") {
    ...
    let keeps = buildKeepSegments(silences, meta.duration, { padding: s.padding });
  ...
  job.status = "planning";
  const plan = await planEdit(transcript, s.instruction, s.targetDuration, meta.duration);
...
async function renderJob(job, keeps) {
  ...
  if (job.mode === "ai" && s.captions) {
    if (s.captionStyle === "bold") job.ass = buildAss(job.words, keeps, { vertical: s.vertical });
  ...
  const needsFinish = job.mode === "ai" && ((s.captions && (job.srt || job.ass)) || s.musicPath || s.vertical);
  const cutTarget = needsFinish ? path.join(OUTPUT_DIR, job.id + ".cut.mp4") : job.output;

  job.status = "cutting";
  const wholeVideo = keeps.length === 1 && keeps[0].start === 0 && keeps[0].end === duration;
  if (wholeVideo) fs.copyFileSync(job.input, cutTarget);
  else
    await cutVideo(job.input, cutTarget, keeps, (sec) => {
  ...
  if (needsFinish) {
    ...
    fs.unlink(cutTarget, () => {});
    for (const f of [srtFile, assFile]) if (f) fs.unlink(path.join(OUTPUT_DIR, f), () => {});
  }

  job.status = "done";
  job.progress = 100;
  job.versions = job.versions || [];
  job.versions.push({ v: job.version, keptDuration: job.keptDuration, cuts: keeps.length, createdAt: Date.now() });
...
function buildBlocks(keeps, words, duration) {
  ...
  for (const k of keeps) {
    if (k.start > cursor + 0.05) regions.push({ start: cursor, end: k.start, type: "cut" });
    regions.push({ start: k.start, end: k.end, type: "keep" });
    cursor = k.end;
  }
  if (duration - cursor > 0.05) regions.push({ start: cursor, end: duration, type: "cut" });
...
function sanitizeSegments(list, duration) {
  const clean = (Array.isArray(list) ? list : [])
    ...
    .filter((s) => isFinite(s.start) && isFinite(s.end) && s.end - s.start >= 0.2)
...
app.post("/api/jobs/:id/render", (req, res) => {
  ...
  const keeps = sanitizeSegments(req.body.included || job.plannedKeeps, job.meta.duration);
  if (!keeps.length) return res.status(400).json({ error: "Nothing selected to keep." });
...
app.get("/api/jobs/:id", (req, res) => {
  const job = jobs.get(req.params.id);
  if (!job) return res.status(404).json({ error: "Job not found." });
  const { id, status, progress, error, duration, keptDuration, segments, originalName, summary, mode } = job;
  const payload = { id, status, progress, error, duration, keptDuration, segments, originalName, summary, mode };
  if (status === "review" || status === "done") payload.reviewBlocks = job.reviewBlocks;
  if (job.transcriptText) payload.transcript = job.transcriptText.slice(0, 6000);
  if (job.versions) { payload.versions = job.versions; payload.version = job.version; }
  res.json(payload);
});
...
app.listen(PORT, () => {
  ...
  console.log(process.env.GROQ_API_KEY ? "AI mode: ready ✓" : "AI mode: OFF — add GROQ_API_KEY to .env");
});
